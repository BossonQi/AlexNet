...
batch_size=32#每轮迭代的样本数(池大小)
num_batches=100#训练次数
...
def inference(images, keep_prob):
  ...
  # pool5
    ...
    flattened = tf.reshape(pool5, [-1, 6*6*256])#对pool5输出的tensor进行变形，将其转化为1D的向量，然后连接全连接层

  # dense1
    with tf.name_scope('dense1') as scope:
        W_dense1 = weight_variable([6*6*256, 4096])#将6x6x256个神经元与4096个神经元(输出)全连接
  ...
  # dense3(softmax)
    with tf.name_scope('dense3') as scope:
        W_dense3 = weight_variable([4096, 1000])###最后输出为1000类
        b_dense3 = bias_variable([1000])
...
def run_benchmark():
    ...
    # 用前面定义的inference函数构建整个网络，得到最后一层的输出和整个网络所需要训练的参数集合parameters
        dense3, parameters = inference(images, 1.0)
    ...
    # 评测反馈计算Backward
        objective = tf.nn.l2_loss(dense3)#计算损失loss
